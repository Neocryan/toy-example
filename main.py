...
p1 = figure(title="Interactive Line Plot", tools="tap",
        tooltips=[("x", "$x{0}")])
p1.x_range = Range1d(start=0, end=10)
p1.y_range = Range1d(start=0, end=10)
# add a line renderer with the data source
line1 = p1.line('x', 'y', source=source1, line_width=2)
circle_glyph = p1.circle('x', 'y', source=source1, size=10, fill_color="white", line_color="red")
...
callback1 = CustomJS(args=dict(source=source1, source2=source2), code="""
